...
{
     __m128i tmpdata0 = _mm_loadu_si128(rte_pktmbuf_mtod_offset(pkt, __m128i *,
 ...
 */
static inline int
port_init(uint16_t port, struct rte_mempool *mbuf_pool, uint16_t rx_rings, uint16_t tx_rings, bool disable_rss)
{
...
lcore_work(void* arg)
{
	uint8_t queue = (uint8_t) (uint64_t) arg;
	...
	printf("Starting core %u with queue %u\n", lcore_id, queue);

	warn_if_not_same_numa(0);

	/* Run until the application is quit or killed. */
	while (!quit) {
		const uint16_t nb_rx = rte_eth_rx_burst(0, queue,
				bufs, BURST_SIZE);
...
soft_lb_lcore_work(void* arg)
{
	...
	while (!quit) {
		...
		const uint16_t nb_tx = rte_ring_sp_enqueue_burst(tx_ring, (void*) bufs, nb_rx, NULL);
...
static int
batch_load_balancer(uint16_t nb_cores, struct rte_ring **rx_rings, struct rte_ring **tx_rings)
{
	struct rte_mbuf *bufs[BURST_SIZE];
	unsigned lcore_id;
	uint64_t rx_stats, tx_stats;
	...
	while (!quit) {
		...
		uint16_t nb_tx = rte_ring_sp_enqueue_burst(rx_rings[current_core], (void*) bufs, nb_rx, NULL);
		...
		nb_rx = rte_ring_sc_dequeue_burst(tx_rings[current_core], (void*) bufs, BURST_SIZE, NULL);
...
static int
hash_load_balancer(uint16_t nb_cores, struct rte_ring **rx_rings, struct rte_ring **tx_rings)
{
	unsigned lcore_id;
	uint64_t rx_stats, tx_stats;
	...
	while (!quit) {
		...
		for (uint16_t i = 0; i < nb_cores; ++i) {
			uint16_t nb_tx = rte_ring_sp_enqueue_burst(rx_rings[i], (void*) bufs[i], rx_by_core[i], NULL);
			/* Free any unsent packets. */
		...
		nb_rx = 0;
		for (uint16_t i = 0; i < nb_cores; ++i) {
			nb_rx += rte_ring_sc_dequeue_burst(tx_rings[i], (void*) (bufs[0] + nb_rx), BURST_SIZE, NULL);
		}
...
main(int argc, char *argv[])
{
	...
	unsigned mbuf_entries = nb_ports * slave_lcore_count * RX_RING_SIZE +
                            nb_ports * slave_lcore_count * BURST_SIZE +
                            nb_ports * slave_lcore_count * TX_RING_SIZE +
                            slave_lcore_count * MBUF_CACHE_SIZE;
	...
	if (port_init(0, mbuf_pool, slave_lcore_count, slave_lcore_count, soft_lb) != 0)
	...
	if (soft_lb) {
		...
		rx_rings = rte_zmalloc("rings",
			sizeof(struct rte_ring*) * slave_lcore_count * 2, RTE_CACHE_LINE_SIZE);
		if (rx_rings == NULL) {
		...
		RTE_LCORE_FOREACH_SLAVE(lcore_id) {
			...
			rte_eal_remote_launch(soft_lb_lcore_work, (void*) &ring_pairs[i], lcore_id);
			++i;
		}
		...
		uint8_t queue = 0;
		RTE_LCORE_FOREACH_SLAVE(lcore_id) {
			rte_eal_remote_launch(lcore_work, (void*) (uint64_t) queue, lcore_id);
			++queue;
		}
