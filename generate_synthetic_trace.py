...
def main():
    if '--auto' in sys.argv:
        pkt_size = 64
        nb_src = 1
        nb_dests = [1, 2, 4, 8, 16, 32, 64, 128, 512, 8192]

        for nb_dest in nb_dests:
            nb_pkts = nb_src * nb_dest
            out_pcap = '_'.join(
                str(x) for x in (nb_pkts, pkt_size, nb_src, nb_dest)
            ) + '.pcap'
            print('Generating ', out_pcap)
